...
class SocketImapLib
{
	 ...
	 */
	public function getMailIsUid($intMailId) {
        if (empty($this->Select_Box)) {
            $this->selectFolder();
        }
	...
	public function getMailTopMessage($intMailId, $intReturnType=1)
	{
     ...
     * @return bool
     */
	public function realDelMail($folder, $intMailId)
	{
		if (!$this->getIsConnect() && $this->bolIsLogin)
		{
			return false;
     ...
     * @return bool
     */
	public function moveMail($folder, $intMailId, $to_folder)
    {
        if (!$this->getIsConnect() && $this->bolIsLogin)
        {
            return false;
	...
	public function renameFolder($oldfolder, $new_folder)
    {
        if (!$this->getIsConnect() && $this->bolIsLogin)
        {
            return false;
